...
export async function main(ns) {
  ...
  while (true) {
    ...
    if (player.isWorking) {
      ...
      ns.print(`Performance: ${player.jobPerformance.toFixed(2)} | Time: ${formatTime(player.timeWorked)}`);
      ns.print(`Exp Gain: H:${player.workHackExpGain}/s S:${player.workStrExpGain}/s D:${player.workDefExpGain}/s`);
    }
    ...
    if (showDetailedStats) {
      ...
      if (purchasedServers.length > 0) {
        ...
        ns.print(`Purchased Servers: ${purchasedServers.length} | Total RAM: ${totalRam.toFixed(1)} GB`);
        ns.print(`RAM Usage: ${usedRam.toFixed(1)}/${totalRam.toFixed(1)} GB (${((usedRam/totalRam)*100).toFixed(1)}%)`);
      }
    }
    ...
    for (const hostname of allServers) {
      ...
      for (const script of runningScripts) {
        if (script.filename !== 'utility-monitor.js') { // Don't count this script
          totalScripts++;
    ...
    ns.print(`Total Scripts: ${totalScripts} | Income: $${ns.formatNumber(totalIncome)}/s | Exp: ${ns.formatNumber(totalExp)}/s`);

    // Network map (if requested)
    if (showNetworkMap) {
      ns.print(`\n=== NETWORK MAP ===`);
      displayNetworkMap(ns);
    }
 ...
 */
function getAllServers(ns, startServer = 'home') {
