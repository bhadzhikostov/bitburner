...
export async function main(ns) {
  ...
  while (true) {
    ...
    for (const symbol of symbols) {
      ...
      ns.print(`${symbol}: $${ns.formatNumber(price)} | Forecast: ${(forecast * 100).toFixed(1)}% | Vol: ${(volatility * 100).toFixed(1)}%`);

      if (shares > 0) {
        const profit = (price - avgPrice) * shares;
        const profitPercent = ((price - avgPrice) / avgPrice) * 100;
        ns.print(`  Position: ${ns.formatNumber(shares)} shares | Profit: $${ns.formatNumber(profit)} (${profitPercent.toFixed(1)}%)`);
        ...
        if (forecast > sellThreshold || profitPercent > 20) {
          ...
          if (ns.sellStock(symbol, shares)) {
            ns.tprint(`Sold ${ns.formatNumber(shares)} shares of ${symbol} for $${ns.formatNumber(price * shares)}`);
          }
        }
      } else {
        ...
        if (forecast < buyThreshold && availableMoney > 100000) {
          ...
          if (sharesToBuy > 0) {
            ns.print(`  Buying ${symbol} - Low forecast, investing $${ns.formatNumber(investment)}`);
            if (ns.buyStock(symbol, sharesToBuy)) {
              ns.tprint(`Bought ${ns.formatNumber(sharesToBuy)} shares of ${symbol} for $${ns.formatNumber(investment)}`);
            }
          }
        }
      }
...
/**
 * Get stock position information
 * This is a helper function since getStockPosition might not be available in all versions
 */
function getStockPosition(ns, symbol) {
  try {
    return ns.getStockPosition(symbol);
  } catch (e) {
    // Fallback: return [0, 0] if function not available
    return [0, 0];
  }
}
