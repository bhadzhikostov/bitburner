    ...
    for (const serverName of purchasedServers) {
      ...
      if (currentRam < targetRam) {
        ...
        if (availableMoney >= upgradeCost) {
          ...
          ns.print(`Need $${ns.formatNumber(upgradeCost)} to upgrade ${serverName} to ${targetRam}GB RAM`);
        }
      }
    }
...
/**
 * Get the cost to upgrade a purchased server to a specific RAM amount
 */
function getPurchasedServerUpgradeCost(ns, hostname, ram) {
  try {
    return ns.getPurchasedServerUpgradeCost(hostname, ram);
  } catch (e) {
    // Fallback calculation if function not available
    const currentRam = ns.getServer(hostname).maxRam;
    const baseCost = 55000; // Base cost for 1GB server
    const ramMultiplier = Math.pow(1.0035, ram - 1);
    const currentCost = baseCost * Math.pow(1.0035, currentRam - 1);
    return (baseCost * ramMultiplier) - currentCost;
  }
}
